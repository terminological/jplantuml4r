...
#' This is the main entry point of the jplantuml4r generated R library.
#'
#' @description
...
JavaApi = R6::R6Class("JavaApi", public=list( 
	...
	.reg = list(),
	#' @field PlantUML the PlantUML class constructors and static methods
	PlantUML = NULL,
	...
	#' @return nothing
 	initialize = function(logLevel = "INFO") {
		...
		self$.log = .jcall("org/slf4j/LoggerFactory", returnSig = "Lorg/slf4j/Logger;", method = "getLogger", "jplantuml4r");
		...
		self$.toJava = list(
			...
			PlantUML=function(rObj) return(rObj$.jobj),
		...
		self$.fromJava = list(
			...
			PlantUML=function(jObj) return(PlantUML$new(jObj,self)),
		...
		self$PlantUML = list(
			new = function() {
				...
				tmp_out = .jnew("io/github/io/github/terminological/PlantUML" , check=FALSE);
				self$printMessages()
				.jcheck() 
...
JavaApi$rebuildDependencies = function( ... ) {
	...
	if (!all(file.exists(jars))) {
		...
		warning("The output of jplantuml4r::JavaApi$rebuildDependencies(debug=TRUE) may help diagnose the problem")
	}
...
.checkDependencies = function(nocache = FALSE, ...) {
	package_jar = .package_jars(package_name="jplantuml4r",types="fat-jar")
	package_jar = package_jar[startsWith(fs::path_file(package_jar),"jplantuml4r-main-SNAPSHOT")]
